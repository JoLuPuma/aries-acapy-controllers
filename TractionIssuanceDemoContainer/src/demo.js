...
const config = JSON.parse(fs.readFileSync('src/config.json').toString('utf8'));
const app = new koa();
...
routerInstance.get('/', koaBody(), async ctx => {
    ctx.body = fs.readFileSync('src/webpage.html').toString('utf8');
});
...
async function authenticate() {
    ...
    let result = await axios.post(`http://host.docker.internal:8032/multitenancy/tenant/${config.tenantId}/token`, {
...
async function sendTractionRequest(relativeUrl, body = {}, params = null) {
    ...
    let response = await axios.post(`http://host.docker.internal:8032${relativeUrl}`, body, {
        params,
        headers: {
